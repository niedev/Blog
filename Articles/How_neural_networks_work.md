...
1) We will formulate f<sub>Cb</sub> as a combination of functions like this, then we will use the chain rule to find a formula that allows us to calculate (via the chain rule) the gradient of fCb with respect to the preactivation values ​​of any level l (which we will denote with δ<sup>(l)</sup>).

2) After that we will use the chain rule to calculate the gradient of any level l with respect to W(l) at the point W0(l) considering the combination between the function f_Cb which has as independent variables the preactivation values ​​of level l (which therefore has as gradient δ^((l) )) and the preactivation function of level l which has as independent variables the components of W(l) (which we indicate with a(l)(W(l)1,1, ..., W(l)nl,nl-1)), which therefore, by the chain rule, will be equal to δ^((l) ) multiplied by the gradient of a(l)(W(l)1,1, ..., W(l)nl,nl-1) at the point W0(l) (which is equal to δ^((l) ) ⨂ 〖z^((l−1) )〗_0), finally we will see that this gradient will have, as components, values ​​equal to the partial derivatives with respect to each weight of the level l: 〖〖(f〗_Cb)〗_(〖W^((l))〗_(i,j))^′ (θ_0 ).

3) Finally we will use the same principles of point 2) to calculate the partial derivatives with respect to each bias of the level l: 〖〖(f〗_Cb)〗_(〖b^((l))〗_i)^′ (θ_0 ).

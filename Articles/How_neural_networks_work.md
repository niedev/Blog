...
1) **We will formulate f<sub>Cb</sub> as a combination of functions like this, then we will use the chain rule to find a formula that allows us to calculate** (via the chain rule) **the gradient of f<sub>Cb</sub> with respect to the preactivation values ​​of any level l** (which we will denote with δ<sup>(l)</sup>).

...
Where efc is an expression that has among the operands f(xt1, σ, θ), yt1, f(xt2, σ, θ), yt2, ..., f(xtm, σ, θ), ytm and represents, for each possible combination of the weights and biases of f, the average amount of variation between the values ​​produced by f with input values ​​from the experiments (f(xti, σ, θ)) and the values ​​of the same experiments produced by humans (yti), consequently:

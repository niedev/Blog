...
Using the formula to calculate δ<sup>(l)</sup> we can prove the following equation, which finds **the gradient of the function $`f_Cb(z^{(L)} (a^{(L)} (z^{(L−1)} (a^{(L−1)} (…(z^{(l)} (a^{(l)} (W^{(l)})))))))))`$**, which we defined above, **at the point W<sub>0</sub><sup>(l)</sup>** (which will bring us to the final equation of point 2, to find $`(f_Cb)_{W^{(l)}_{(i,j)}}(θ_0)`$ knowing only the value of $`δ^{(l)}_i`$ and $`z^{(l−1)}_{0j}`$):
